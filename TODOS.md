...
- TODO-10V-2: Database-backed editable pricing. New `AiModelPricingOverride` table (migration). CRUD API. Edit panel in AI Config. `AiPricingOptions` extended to check DB first, fall back to config. Audit fields: `UpdatedByAdminId`, `UpdatedAt`, `EffectiveFrom`, `IsActive`. Identified in Phase 10V-0 audit.
- TODO-10V-3: Zero-cost log alert in AI Usage summary. Warn when active models have no pricing configured (cost=0 due to missing price). Identified in Phase 10V-0 audit.
