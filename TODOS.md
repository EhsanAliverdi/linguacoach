...
### TODO-20I-5 — Wire real TTS audio + real reading passages into live placement items
**What:** `PlacementAudioService` (calls `tts.placement`, generates real speech) and the frontend `PlacementService.getListeningAudioBlobUrl()` already exist but belong to an older, parallel placement API the live UI no longer uses. Wire them into the live adaptive placement engine's listening items using the new `ListeningAudioScript` field (TODO-20I-4, shipped). Similarly, surface `ReadingPassage` in the student-facing placement UI for reading-skill items — the field exists on `PlacementItemDefinition` and is seeded/editable in `/admin/placement-items`, but `PlacementNextItemDto` doesn't include it yet and no admin has authored real passages into it.
**Why:** Confirmed the audio infrastructure isn't missing, just unwired to the currently-live engine — closes finding (6) from the onboarding-cutover review (listening items are text pretending to be audio, reading items are single sentences not passages).
**Context:** `docs/reviews/2026-07-03-phase-20i-onboarding-cutover-and-mc-render-fix-review.md`.
**Deferred from:** Phase 20I, 2026-07-03 (TODO-20I-4's DB-backed foundation now exists; this is the remaining wiring work — new `PlacementAssessmentItem` audio columns, an adaptive-engine audio endpoint, `PlacementNextItemDto` changes, and frontend `<audio>`/passage rendering).
