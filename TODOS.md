...
- TODO-10V-3: Wire `IAiPricingResolver` into `OpenAiProvider`, `GeminiProvider`, `AnthropicProvider` to replace direct `AiPricingOptions` calls. Also add zero-cost log alert in AI Usage summary. Also add frontend override management UI (list, create, edit, deactivate). Identified in Phase 10V-0 audit; deferred from 10V-2.
