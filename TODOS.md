...
- TODO-10Students-F-G: add lifecycle/onboardingStatus/cefrLevel filter selects to the admin students filter bar UI. Backend params already exist (`lifecycleStage`, `onboardingStatus`, `cefrLevel` query params). UI needs three native selects or `sp-admin-select` wrappers wired into `load()`.
