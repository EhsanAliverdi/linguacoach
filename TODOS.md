...
- TODO-10X-G-AICONFIG-FORMS: migrate AI Config page-local `.sp-ai-select` provider/model/voice inputs and the dense provider-credentials grid to `sp-admin-form-field`/`sp-admin-select`/`sp-admin-input`. Deferred again in 10X-G-F due to high field count and ngModel-driven option logic.
...
- TODO-10X-G-INTEGRATIONS-FORMS: migrate Integrations operational forms and legacy table utility classes inside wrapper cards to admin form/table wrappers.
- TODO-10X-FORMS-CVA: add a `ControlValueAccessor` to `sp-admin-input` and `sp-admin-select` so they can two-way bind to parent ngModel/reactive forms. Until then, ngModel-driven admin forms must keep native `.sp-input` controls inside `sp-admin-form-field` (see admin-ui-design-system.md "Form-field wrapper rule"). Blocks full field-level migration of AI Config and Integrations forms.
