...
- TODO-10X-G-AICONFIG-FORMS: migrate AI Config page-local `.sp-ai-select` provider/model/voice inputs and the dense provider-credentials grid to `sp-admin-form-field`/`sp-admin-select`/`sp-admin-input`. Deferred again in 10X-G-F due to high field count and ngModel-driven option logic. **Unblocked by 10X-H** (CVA now exists); remains a dedicated per-field migration pass to avoid silent save regressions in the dense credentials grid.
...
- TODO-10X-G-INTEGRATIONS-FORMS: migrate Integrations operational forms and legacy table utility classes inside wrapper cards to admin form/table wrappers. **Unblocked by 10X-H** (CVA now exists); remains a dedicated per-field migration pass.
