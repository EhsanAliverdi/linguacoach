...
- TODO-10X-G-AICONFIG-FORMS: migrate AI Config page-local `.sp-ai-select` provider/model/voice inputs and the dense provider-credentials grid to `sp-admin-form-field`/`sp-admin-select`/`sp-admin-input`. Deferred in 10X-G due to high field count and ngModel-driven option logic.
- TODO-10X-G-CURRICULUM-FORMS: migrate Curriculum create/edit and routing-preview `.sp-input` fields (inside the new `sp-admin-card`) to `sp-admin-form-field`/`sp-admin-select`.
