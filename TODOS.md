...
### TODO-D4-1 — Context tags only affect full reading passages in Today bank-first selection
**What:** Phase D4's "general English by default, workplace only when routed" context filter in `TodayBankResourceSelector` can only be applied to full `CefrReadingPassage` resources, because that is the only published bank entity that stores context tags. `CefrVocabularyEntry`/`CefrGrammarProfileEntry`/`CefrReadingReference` carry no context-tag columns on the final tables (the tags exist on the staging `ResourceCandidate` but are dropped at publish time), so supporting vocabulary/grammar/short-reference selection cannot yet be context-filtered — a general learner could still receive workplace-leaning vocabulary if the bank held any (the E6/E8 packs keep workplace a minority, which mitigates this today).
**Why:** Closing this would let the workplace/general context default apply uniformly across all bank types, not just passages. It needs a schema change (add context-tag columns to the three lean bank entities + carry them through `ResourceCandidatePublishService`) plus a query-surface change to expose/filter them — deliberately out of scope for D4, which was a selector/composer expansion, not a bank-schema change.
**Context:** `docs/architecture/learning-activity-engine.md` (Phase D4 section); `ResourceCandidatePublishService.BuildTargetEntity`.
**Deferred from:** Phase D4, 2026-07-09.
