...
- TODO-10W-5C-DEFERRED: DB-backed notification channel enable/disable toggles — appsettings-only in 10W-5C. A future phase should introduce a `NotificationChannelConfig` table so channels can be toggled without a redeploy. PUT endpoint deferred.
...
- TODO-10W-5D-RESET-INTEGRATION: Wire `PasswordResetHandler` to render and use the `account.password_reset` Email template instead of hard-coded subject/body. Low-risk; template foundation is ready. Deferred from 10W-5D to avoid touching password reset flow mid-phase.
...
- TODO-10W-PREFS: Notification preferences — `NotificationPreference` per-user per-channel opt-out entity + migration. Respected in `NotificationDispatchService`. Admin UI for viewing/editing per-student preferences. Deferred from 10W-5D.
