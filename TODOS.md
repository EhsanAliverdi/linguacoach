...
- TODO-10W-5B: Admin Send Notification — admin-initiated notification to one or all students. Single endpoint `POST /api/admin/notifications/send`. Channel: InApp or Email. Subject/body fields. Target: userId or broadcast. Frontend form in notification center page.
- TODO-10W-5C: Notification configuration — per-channel enable/disable toggle in admin. Respect channel-disabled flag in dispatch worker.
- TODO-10W-5D: Templates and preferences — `NotificationTemplate` entity + migration. `INotificationTemplateRenderer` (Liquid or Handlebars). Admin CRUD for templates. `NotificationPreference` per-user per-channel. Opt-out respected in dispatch worker.
