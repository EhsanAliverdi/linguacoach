...
- TODO-10W-4b: Token-based self-service password reset link — `GeneratePasswordResetTokenAsync` → email with reset link → `ResetPasswordAsync` endpoint. Deferred: current admin-set temp-password flow works; self-service link requires a reset endpoint + token expiry + frontend reset-password page. Safe gap — document for next auth sprint.
