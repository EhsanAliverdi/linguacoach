...
- TODO-10W-5C-3-KEY-PERSISTENCE: Configure Data Protection key persistence for production. Currently keys are ephemeral — container restarts invalidate stored ciphertext. Must add `PersistKeysToFileSystem` or a cloud key store (Azure Key Vault / AWS Secrets Manager) before production deploy.
