...
- TODO-10W-5C-2-ENCRYPTION: Upgrade `SecretEncrypted` from Base64 to real AES-256 or DPAPI encryption in `NotificationChannelConfig`. Currently encoded only — readable if DB is compromised. Requires adding encryption infrastructure to Infrastructure layer.
- TODO-10W-EMAIL-SENDER-RUNTIME: Wire `SmtpEmailSender` to read DB config at send time via the config resolver, not just at DI resolution time from `IOptions<EmailOptions>`. Currently DB config affects the admin view only; the live sender still uses appsettings. Needs `INotificationChannelConfigResolver` introduced and injected into `SmtpEmailSender`.
