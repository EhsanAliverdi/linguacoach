...
### TODO-E9-1 — Today selector does not yet consume the new lean-table metadata filters (Phase D5)
**What:** Phase E9 made `CefrVocabularyEntry`/`CefrGrammarProfileEntry`/`CefrReadingReference` filterable by context/focus/subskill/difficulty on the published rows, but `TodayBankResourceSelector` still only applies the general-English/workplace context filter to full passages (its D4 behavior). Wiring the selector to pass context/focus/subskill/difficulty filters into `ResourceBankQueryService` for supporting vocabulary/grammar/references — so a general learner never receives workplace-leaning supporting content and topic matching improves — is **Phase D5 (Context-Aware Today Bank Selection and Topic Matching)**, the likely next phase.
**Why:** E9 was deliberately scoped to the producer side (schema/mapping/backfill/query), keeping the consumer-side selector change isolated in its own tested phase.
**Context:** `docs/architecture/learning-activity-engine.md` (Phase D4 section, `TODO-D4-1` note); `TodayBankResourceSelector`.
**Deferred from:** Phase E9, 2026-07-09.
