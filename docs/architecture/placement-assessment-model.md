...
The listening section (Section 4) uses server-side TTS audio generated by `PlacementAudioService`.

- Audio is generated once per assessment when the student first loads the listening section
- Stored under `{Tts:AudioStoragePath}/placement/{assessmentId}/listening.wav`
- Served via `GET /api/placement/audio/{assessmentId}/listening` (authenticated, ownership-checked)
- `PlacementCurrentSectionDto` returns `audioUrl` and `audioAvailable` for the listening section
- If TTS generation fails, placement still loads; frontend shows fallback message and optional browser SpeechSynthesis
- Browser `SpeechSynthesis` is a last resort only — the primary path is the server-generated audio file
- Uses `ITextToSpeechService` abstraction; `FakeTextToSpeechService` in dev/test (silent WAV), real provider configurable via `TTS_PROVIDER` env var

See: [server-side-tts-placement-listening-sprint.md](../sprints/server-side-tts-placement-listening-sprint.md)
