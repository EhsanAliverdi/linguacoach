...
**Prompt key naming convention:** `activity_generate_{activityType}`, `activity_evaluate_{activityType}`.

Example: `activity_generate_writing`, `activity_evaluate_writing`.
...
### 4. New Angular component

Each activity type renders differently. Add a new standalone component (e.g. `SpeakingSessionComponent`) and extend the `ActivityLessonComponent` to route to it based on `activityType`.

The existing `ActivityDto` carries the `activityType` field — the Angular router or the lesson component uses it to decide which child to render.
