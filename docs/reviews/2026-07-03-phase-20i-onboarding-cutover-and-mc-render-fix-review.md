...
## Addendum, 2026-07-03: Phase 6 (TTS usage-logging gap) and Phase 4 (placement item bank) shipped

**Phase 6 — corrected diagnosis of the "Today-lesson TTS gap".** The original audit's claim (MinIO has no audio files, TTS never invoked) was stale. Live reproduction proved TTS generation genuinely works — a real ~970KB Gemini-generated WAV file was confirmed playable with correct duration/voice metadata. The actual bug: `ListeningAudioService.EnsureAudioAsync` and `PlacementAudioService.EnsureListeningAudioAsync` both call `ITextToSpeechService.GenerateSpeechAsync` directly, bypassing `AiExecutionService`'s shared usage-logging wrapper, so `ai_usage_logs` never got a row for `tts.listening`/`tts.placement` despite the calls succeeding. Fixed by logging usage directly in both services (0 tokens/cost, since TTS isn't token-priced in this codebase). No user-facing behavior changed. Shipped in commit `94dfd96`.

**Phase 4 — admin-configurable placement item bank.** Replaced `PlacementAssessmentService`'s static 72-item `ItemBank` with a DB-backed `PlacementItemDefinition` entity, mirroring the onboarding-step admin CRUD pattern exactly: new entity + EF config + migration (`placement_item_definitions`, unique index on `Prompt` — the same string the adaptive selection logic already used as de-facto item identity), `PlacementItemBankSeeder` (idempotent, admin-edit-safe backfill of the original 72 items), `AdminPlacementItemController` at `/api/admin/placement-items` with add/update/remove/list handlers, and a new admin page at `/admin/placement-items`. Added `ReadingPassage`/`ListeningAudioScript` fields to the entity now, ahead of TODO-20I-5's remaining wiring work, so admins can start authoring richer content immediately. `PlacementAssessmentService`'s adaptive selection algorithm itself is unchanged — it now loads the enabled item bank from the DB once per outer call instead of reading a static field. Shipped in commit `c8a8c6c`; full backend suite (3170 tests, ArchitectureTests 5 / UnitTests 1757 / IntegrationTests 1408) and new Karma specs (26 tests) green.

**Phase 5 — real audio + reading passages wired into live placement.** `PlacementAssessmentItem` (the per-attempt issued item) gained `ReadingPassage`/`ListeningAudioScript` (copied from `PlacementItemDefinition` at issuance) and `AudioStorageKey`/`AudioContentType` (populated lazily on first request). New `AdaptivePlacementAudioService` mirrors `ListeningAudioService`'s shape rather than reusing the legacy `PlacementAudioService`, which hand-rolls local-filesystem storage that doesn't work with Minio in production — the new service uses `IFileStorageService` instead, and logs TTS usage directly (same gap as Phase 6, since TTS calls bypass `AiExecutionService`). New endpoint `GET /api/student/placement/audio/{assessmentId}/items/{itemId}/listening` on `StudentPlacementController` (the live adaptive controller — distinct from the unused legacy `PlacementController`). `PlacementNextItemDto` gained `ReadingPassage`/`HasAudio`; `PlacementComponent` renders an `<audio>` player and a passage box. `PlacementItemBankSeeder` derives each listening item's audio script from its prompt's quoted "You hear: '...'" text (substituting gap-fill blanks with the correct answer), and retroactively backfilled the script onto the 72 rows the Phase 4 deploy had already seeded without it. Shipped in commit `6774c10`; full backend suite (3180 tests) and Karma specs (49 across the two touched components) green. Reading passages remain only as rich as an admin authors them — the seeded defaults still have `ReadingPassage = null`, since deriving one from the existing single-sentence prompts wouldn't have improved content depth.

See `TODOS.md` TODO-20I-4 and TODO-20I-5 for the resolved-state summaries.
