...
**Deferred:** TODO-20I-5 (wire `PlacementAudioService`/`ListeningAudioScript` into the live adaptive engine's listening items, surface `ReadingPassage` to the student-facing reading UI) and Phase 4 originally scoped this as a single phase — split after Phase 4 landed, since the DB-backed foundation existing doesn't yet mean anything reads the new fields at assessment time. See `TODOS.md` TODO-20I-5.
