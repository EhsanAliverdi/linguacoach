...
**Accurate as of: 2026-07-09 (Phase G1 — see §19a for the current phase sequence).
...
Phase D2, Plan-Sync-After-D2, Phase E7, Plan-Sync-G0, Phase G0, Plan-Sync-After-G0, Phase G1) has
...
**Latest phase completed (local, not yet deployed):** Phase G1 —
Admin Information Architecture Cleanup (2026-07-09). Implemented the low-risk admin-IA quick wins
...
**Latest phase completed before this:** Plan-Sync-After-G0 —
...
## 19a. Phase Sequence (as of 2026-07-09, Phase G1)
...
18. **Phase E8** — further resource-platform hardening/enrichment (RAG/search, larger import support) if needed after E7 — proceed depending on what E7 reveals is still missing. Available now that G1 is done; gated on the Phase E8/D3 decision checkpoint below. Not started.
19. **Phase E8/D3 decision checkpoint** — **now live (after Phase G1), not resolved in advance**: start Phase D3 (broader Today composer migration) using the deepened bank content, continue Phase E8 for resource depth, continue with Practice Gym v2 (PG-v2A), or do Phase G2/G3 (backend/diagnostics cleanup) next. Not reached yet.
...
and the legacy `IAiActivityGenerator` path are all kept.** **Phase G1 is complete.** A Phase
E8/D3 decision checkpoint now applies (item 19 above), not resolved in advance: Phase D3, Phase
E8, Phase G2/G3 (backend/diagnostics cleanup), or PG-v2. **Full Phase D implementation (beyond
D1/D2's narrow slice) has not started. Phase E8, Phase G2/G3, and PG-v2 implementation have not
started.**
