...
| 2026-07-21 | **Platform Reliability Sprint 14.2 implemented — Skill Graph "Content coverage" full redesign + first pass of an app-wide admin table consistency effort.** User feedback: the Content coverage sub-table had no pagination, no row detail, a confusing "Re-tag next batch" flow, and didn't match the visual conventions used elsewhere (Resource Bank's table, the AI Config page's "Pricing overrides"/model-pricing header+table). (1) `GET .../skill-graph/content-coverage` rewritten to return **every** approved node (not just the zero-content gap list) with its real linked-Module id/title list and tags, so the frontend can render one real browsable table instead of a dead-end gap dump; +2 integration tests updated for the new `nodes[]` shape (replacing the old `nodesWithoutContent[]`). (2) Frontend: client-paginated (20/page) table styled exactly like Resource Bank's (badges, fluid title column, `sp-admin-table-footer` + `sp-admin-pagination`), row click opens a `sp-admin-slide-over` showing the node's tags and its real linked Modules as router links to `/admin/modules/{id}` — closing the "click a row for more info" gap. (3) Fixed the confusing "Re-tag next batch" flow: it was silently sweeping *untagged Modules* (a different set from whatever the coverage table showed as gaps), so a "0 links applied" result looked broken even when correct. Backend now also returns `remainingUntaggedModuleCount` (re-queried fresh after the sweep's own links save) and the button/copy were reworded to make the actual scope explicit, rather than changing the underlying sweep mechanism itself. (4) **Standardized the "titled section" header** (`sp-admin-section-header`, added in Sprint 14.1 but never actually adopted anywhere despite existing) into the AI Config page's "Pricing overrides"/per-provider/"Config pricing"/"LLM categories"/"TTS categories"/"Provider credentials" headers, replacing 6 sites of hand-rolled `.sp-admin-card-title`/`.sp-admin-provider-header`/`.sp-admin-section-header-row` markup — **first attempt broke the visual result** (flagged live by the user: the tinted-background/padding/border-bottom "bar" look on the per-provider "ANTHROPIC" header was lost) because the component only had one plain, unstyled treatment; fixed by giving it a `variant: 'bar' | 'plain'` input matching the two visual patterns that already existed in this codebase (`bar` — background+padding+border, for a `padding="none"` card's `[slot=header]`, restoring `.sp-admin-provider-header`'s exact styling byte-for-byte; `plain` — the existing no-background floating description row, for headers that sit above a section rather than inside a bordered card). (5) Applied the same `padding="none"` + bar-header + `[flush]="true"` table pattern to Skill Graph's Content coverage card, matching the AI Config model-pricing table's own "no left/right border" look exactly, per explicit user instruction. (6) Standardized Resource Bank's bulk-select checkboxes from raw `<input type="checkbox">` to `sp-admin-checkbox` (matching Skill Graph's Nodes table), adding the previously-missing header select-all wiring. **Found and fixed 3 more pre-existing (not newly introduced) Karma bugs while touching these files, confirmed via `git show HEAD` before fixing**: `admin-ai-config`'s `saveOverride` empty-modelName test asserted a message the code never actually returns; `admin-resource-bank-unified`'s spec never provided `AdminResourceCandidateService` (added in Sprint 12) or `issuesSummary()` on its bank-service mock, both silently missing since those features landed; a positional-args assertion on `list()` never updated for Sprint 12's `archivedOnly`/`unusedOnly` params. Frontend Karma improved from the prior 161 pre-existing failures to 154 (net improvement, zero regressions, confirmed via 3 successive full-suite runs during this fix cycle). Full backend suite green throughout (30 architecture + 1,315 integration + 2,432 unit). **Verified live against the Docker dev DB and the rebuilt `--no-cache` containers**: confirmed the deployed bundle contains the fixed `sp-sh-root--bar` CSS class. | Made an unforced, real mistake this sprint: styling `sp-admin-section-header` with only one plain visual treatment and rolling it out to 6 sites without checking each site's actual prior visual context (padding-none card header vs. floating row) — caught immediately by the user's live screenshot rather than by any test (Karma doesn't assert computed CSS), which is a real gap in this project's visual-regression coverage worth flagging for a future session, not something this fix pretends to close. Fixed by encoding the two pre-existing patterns as an explicit `variant` input rather than picking one look and forcing every site to match it, since both patterns were legitimate, established, and used in different structural contexts (bordered padding-none card header vs. inline section description) — collapsing them into one look would have been a real regression for whichever context lost its pattern, not a consolidation. Chose to fix the 3 newly-discovered pre-existing Karma bugs immediately rather than deferring them, since they were direct consequences of files this exact change set was already touching and each was a mechanical, low-risk fix (add a missing mock, correct a stale assertion) — consistent with this session's established "fix what a live/test verification step proves is broken" discipline, not scope creep |
| 2026-07-21 | **Platform Reliability Sprint 14.3 implemented — Skill Graph "Nodes" card redesign to match the same style.** Live feedback: Sprint 14.2 fixed Content coverage but the Nodes card (the page's main table, with the Table/Graph toggle) was left on the old plain `sp-admin-card title="Nodes"` pattern. Converted it to the same `padding="none"` + bar-header + `[flush]="true"` table + `sp-admin-table-footer` structure as Content coverage and Resource Bank, with the Table/Graph toggle buttons moved into the header's `[slot=actions]` and a real node-count badge (added `nodesTotalCount` signal, populated from the existing `SkillGraphNodeListResponse.totalCount` the API already returned but the component never stored). The filter bar, bulk-approve/reject controls, and status alerts — previously flush against the old card's own padding — now sit in an explicit padded wrapper div since the card itself is padding-none, while the table and its footer/pagination remain edge-to-edge outside that wrapper, matching Content coverage's structure exactly. Frontend Karma unaffected (touched spec still 13/13 in isolation); full-suite pass/fail counts fluctuated within this suite's already-documented pre-existing flakiness band (154–174 across today's runs), not attributable to this change. Verified live: deployed bundle confirmed to contain the new `[flush]` table markup. | Kept the Table/Graph toggle in the header's actions slot rather than as a separate row inside the body, since it's a page-level view-mode control for the whole card (both branches), not something scoped to just the table body — putting it in the header keeps it visible and consistent regardless of which view is active, matching how Content coverage's "Sweep next batch" action lives in its own header, not its body |
| 2026-07-23 | **Skill Graph rebuild Phase 1 implemented — node/edge editability + isolated-node metric.** First of 6 planned phases (`docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md`'s follow-up plan) fixing the gaps that audit found. `SkillGraphNode.UpdateCore(...)` added — same reject-first-to-edit convention as `Module`/`Lesson`/`Exercise.UpdateDraft` (blocked while Approved, editing a Rejected node resubmits it to PendingReview); `Key` stays stable/uneditable, confirmed safe since routing/mastery/learning-plan all resolve nodes live by `Key` on every call. `AdminSkillGraphController` gained: `POST nodes` (manual create, starts PendingReview), `PUT nodes/{id}`, `POST`/`DELETE nodes/{id}/prerequisites` (manual edge add/remove, cycle-validated via `ISkillGraphValidationService` against the FULL active graph, not a batch subset), `GET nodes/isolated` (nodes with zero edges on both sides — the real defect signal behind the audit's screenshot), and `POST nodes/import` (idempotent bulk upsert-by-Key, resolving `PrerequisiteKeys` against the full active graph — this is what makes cross-Skill/cross-CEFR-level edges structurally possible for the first time, the actual root cause the audit traced; it's also the ingestion mechanism Phase 4's canonical curriculum reseed will use). `BatchReject` now cascade-deletes any edge touching a rejected node instead of leaving it dangling. Frontend: new "Create node" modal and node detail slide-over (Prerequisites/Unlocks lists, add-prerequisite search, Edit button) on `/admin/skill-graph`, plus a new dedicated `/admin/skill-graph/nodes/:id/edit` route mirroring `admin-module-edit`'s established pattern exactly. **Found and fixed while testing**: a SQLite LINQ-translation error in the new isolated-nodes query (`SelectMany` over an in-memory array projection isn't translatable — fixed by materializing edge endpoints first, then computing distinct ids client-side); and confirmed `admin-skill-graph.component.spec.ts` was silently broken on `main` before this session (calling nonexistent `toggleSelected`/`isSelected` methods) — fixing it exposed a second latent bug (missing `provideHttpClient()` in the test bed, needed by `SpAdminHelpIconComponent`'s `AdminHelpContentService`), plus 4 unrelated pre-existing spec bugs in `admin-ai-usage`/`admin-diagnostics`/`admin-exercise-types`/`admin-placement-items` (stale property/method names) that were silently killing the entire Karma suite's ability to even load, matching this session's own repeatedly-documented "one broken spec kills the whole suite" pattern — all fixed since they were direct, mechanical, low-risk blockers to verifying this phase's own new tests. +7 unit tests (`SkillGraphNodeTests`), +16 integration tests (`AdminSkillGraphEndpointTests`), +7 new/fixed component tests. **Verified**: full backend suite 30 architecture + 2,436 unit + 1,328 integration, all green; frontend production build clean; full Karma suite went from a complete load failure on `main` to 1,478 passing / 237 failing (pre-existing, confirmed none touch any file this phase changed) — not yet deployed live. Phase 2 (widen AI-drafting's edge-resolution scope) and Phase 3 (canonical curriculum authoring) are next | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and the approved implementation plan referenced there. Next: Phase 2 (cross-category AI-draft edge resolution), then Phase 3 (canonical CEFR-standard curriculum authoring, book-level granularity, full wipe-and-reseed per user decision) |
| 2026-07-23 | **Skill Graph rebuild Phase 2 implemented — cross-Skill/cross-CEFR-level AI-draft edge resolution.** Second of 6 planned phases. `AdminSkillGraphController.Draft()` previously resolved `PrerequisiteTitles` only against nodes sharing the exact same `(CefrLevel, Skill)` as the request — the confirmed root cause of the audit's isolated-category-islands screenshot. Now also queries real `Approved && IsActive` "cross-link candidates" (same skill/other CEFR level, e.g. A1→A2 grammar progression; OR same CEFR level/other skill, e.g. A1 grammar→A1 speaking — the two exact shapes the audit named), bounded to 40 (`MaxCrossLinkCandidates`, mirrors `MaxExistingTitles`'s bounded-call discipline), and passes their titles into a new `crossLinkCandidates` AI prompt variable. `SkillGraphDraftRequest` gained the field; `SkillGraphDraftingService` threads it into the AI context; `DefaultAiSeeder`'s `skill_graph_propose_nodes` prompt template and instructions updated (auto-upgrades via the seeder's existing content-hash mechanism on next API start), `maxInputTokens` raised 1800→2600 with real headroom for the added candidate list, matching the Sprint 9/14 token-budget-fix precedent. Prerequisite resolution now checks three sources — same-combo existing, same-batch, and cross-link candidates — the same three-source discipline Phase 1's `ImportNodes` already established; cycle validation widened from a batch-scoped node/edge subset to the FULL active graph, matching `AddPrerequisite`'s approach, since a cross-combination edge could close a cycle through a node the batch never directly touched. +2 unit tests (`SkillGraphDraftingServiceTests`, using a new `SwappableFakeAiProvider.LastRequest` capture to assert cross-link titles actually reach the rendered prompt), +1 integration regression test (the new EF query against real seeded cross-skill/cross-level nodes doesn't throw — this session's SQLite `SelectMany`-translation bug from Phase 1 made this worth testing explicitly). Backend suite: 30 architecture + 2,438 unit + 1,329 integration, all green. Not yet deployed live — the new prompt version/token budget only take effect after a fresh API start. Phase 3 (canonical CEFR-standard curriculum authoring) is next | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan. Next: Phase 3 (book-level-granularity curriculum authoring, level-by-level), then Phase 4 (full wipe-and-reseed via Phase 1's `nodes/import` endpoint) |
| 2026-07-23 | **Skill Graph rebuild Phase 1 follow-up — Create node now places the node in the graph at creation time, and moved from modal to slide-over (user correction).** `CreateSkillGraphNodeRequest` gained `PrerequisiteNodeIds`; `AdminSkillGraphController.CreateNode` links each one via a new shared `TryAddPrerequisiteEdgeAsync` helper (factored out of `AddPrerequisite`, which now calls the same helper) — every prerequisite is still cycle-validated against the full active graph, and any that can't be linked (self-reference, not found, would cycle) is reported back as `droppedPrerequisites` (id + reason) rather than silently dropped or failing the whole create. Frontend: the Create panel is now `sp-admin-slide-over` (previously `sp-admin-modal`), gained a "Where does this node go? (prerequisites)" section reusing the same search-and-pick UI pattern the node detail panel already has, backed by a shared `loadNodesForPicker()`. +2 integration tests. Backend 30 architecture + 2,438 unit + 1,331 integration all green; frontend production build clean, component Karma spec 17/17 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan (updated in place with this correction) |
| 2026-07-23 | **Skill Graph rebuild Phase 1 follow-up #2 — symmetric "what does this node unlock?" in Create, and full Prerequisites/Unlocks management added to both View and Edit.** User correction: the graph is genuinely many-to-many in both directions (a node can have several prerequisites AND be the prerequisite for several others), and Create only let an admin pick what a new node depends ON, never what it unlocks; the dedicated Edit route showed neither direction at all, only the View slide-over did (and only read-only for "Unlocks"). `CreateSkillGraphNodeRequest` gained `DependentNodeIds`, linked via the same shared `TryAddPrerequisiteEdgeAsync` helper with arguments swapped (dependent becomes `NodeId`, the new node becomes `PrerequisiteNodeId`) — response gained `droppedDependents` alongside the existing `droppedPrerequisites`. The View slide-over's "Unlocks" list is no longer read-only — same add/remove/search UI as "Prerequisites," reusing the exact same `AddPrerequisite`/`RemovePrerequisite` endpoints with swapped ids (no new backend surface needed for either direction). `admin-skill-graph-node-edit` (previously core-fields-only) gained its own Prerequisites/Unlocks sections with the same pattern; `item` converted from a plain field to a signal so these lists re-render after an edge change. +2 integration tests. Backend 30 architecture + 2,438 unit + 1,332 integration all green; frontend production build clean, component spec 17/17 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan (updated in place with both user corrections) |
| 2026-07-23 | **Skill Graph rebuild Phase 3 — canonical A1 curriculum authored (99 nodes, 0 isolated).** First of 6 CEFR-level authoring passes. `src/LinguaCoach.Persistence/Seed/SkillGraph/a1.json` — 99 hand-authored nodes across all 9 skills (Grammar 20, Vocabulary 20, Reading/Listening/Writing 10 each, Speaking 12, Pronunciation 7, Fluency 5, Confidence 5), built from the standard CEFR progression sequenced the way mainstream coursebooks order A1 topics (per the earlier "CEFR-standard backbone, informed by book structure" decision) — not scraped from any publisher. 105 prerequisite edges, 64 of them cross-skill (Speaking/Writing/Reading/Listening nodes depending on Grammar/Vocabulary; a 7-node internal Pronunciation progression from individual sounds to sentence-level intonation/linking) — this is the direct proof that the isolated-category-island defect Phases 1-2 fixed the *mechanism* for is now also fixed in real *content*. Zero fully-isolated nodes, achieved by an explicit connectivity pass that re-wired 22 nodes which came out isolated on first draft (mostly standalone topical vocab sets — colors, clothes, body parts — added as additional prerequisites of the Speaking/Writing/Reading nodes that naturally use them). New permanent test suite `SkillGraphSeedDataTests` (6 checks, auto-discovers every `*.json` under the seed folder so future levels get the same checks for free) validates: real-contract deserialization, taxonomy compliance, no duplicate/dangling keys, **no cycles** (reuses the real `SkillGraphValidationService` DFS checker), no isolated nodes. Backend 30 architecture + 2,444 unit + 1,332 integration all green. A2-C2 remain — each will cross-link back to real A1 keys once authored. Not yet imported/reseeded into any database — that's Phase 4 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan |
| 2026-07-23 | **Skill Graph rebuild Phase 3 — canonical A2 curriculum authored (100 nodes, 0 isolated, 47 real cross-level edges into A1).** Second of 6 CEFR-level authoring passes. `src/LinguaCoach.Persistence/Seed/SkillGraph/a2.json` — 100 nodes across all 9 skills (Grammar 21, Vocabulary 20, Reading/Listening/Writing 10 each, Speaking 12, Pronunciation 7, Fluency 5, Confidence 5). 122 prerequisite edges: 65 cross-skill, and — new for this pass — 47 genuinely cross-level, referencing real `a1.json` keys (e.g. `grammar.past_simple_regular.a2` depends on `grammar.past_simple_be.a1`), proving the graph now spans CEFR levels the way a real coursebook progression does, not just within one level. Zero isolated nodes, this time resolved proactively during authoring (A1 needed a separate connectivity-fix pass afterward; A2 wired every root vocab/grammar node to a real dependent while writing it, learning from that). `SkillGraphSeedDataTests` upgraded to be cross-file aware — the dangling-key and isolated-node checks now validate against the full merged keyspace across every seed file (checking only within one file produced false positives for A2's legitimate cross-level edges to A1), plus a new whole-graph cycle check across every file combined. 13 seed-data tests, all green. Backend 30 architecture + 2,451 unit + 1,332 integration all green. B1-C2 remain. Not yet imported into any database — Phase 4 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan |
| 2026-07-23 | **Skill Graph rebuild Phase 3 — canonical B1 curriculum authored (101 nodes, 0 isolated, 52 cross-level edges).** Third of 6 CEFR-level authoring passes. `src/LinguaCoach.Persistence/Seed/SkillGraph/b1.json` — 101 nodes across all 9 skills (Grammar 22, Vocabulary 20, Reading/Listening/Writing 10 each, Speaking 12, Pronunciation 7, Fluency 5, Confidence 5). 111 prerequisite edges: 54 cross-skill, 52 cross-level back to real A1/A2 keys (e.g. `grammar.past_perfect_simple.b1` depends on both A2's past simple and past continuous). One node came out isolated on first draft (`confidence.tolerating_ambiguity_in_unfamiliar_topics.b1`) — fixed by wiring it into `fluency.maintaining_a_longer_discussion.b1`. `SkillGraphSeedDataTests` now covers 19 checks across all 3 files, all green. Backend 30 architecture + 2,457 unit + 1,332 integration all green. B2-C2 remain. Not yet imported into any database — Phase 4 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan |
| 2026-07-23 | **Skill Graph rebuild Phase 3 — canonical B2 curriculum authored (100 nodes, 0 isolated, 72 cross-level edges — a real authoring bug caught by the test suite).** Fourth of 6 CEFR-level authoring passes. `src/LinguaCoach.Persistence/Seed/SkillGraph/b2.json` — 100 nodes across all 9 skills (Grammar 21, Vocabulary 20, Reading/Listening/Writing 10 each, Speaking 12, Pronunciation 7, Fluency 5, Confidence 5). 113 prerequisite edges: 42 cross-skill, 72 cross-level back to real A1/A2/B1 keys — the highest cross-level density of any level so far, since much of B2 (mixed conditionals, advanced passive tenses, advanced reporting verbs, inversion, cleft sentences) directly extends B1/A2 structures. **The permanent `SkillGraphSeedDataTests` suite caught a real mistake during this pass**: a prerequisite referenced `grammar.present_simple_negative_questions.a2`, a key that doesn't exist — the real node is `.a1` — caught immediately by the merged-keyspace dangling-key check rather than silently producing a broken import later. Zero isolated nodes on the first pass (no separate connectivity-fix step needed, following A2/B1's proactive-during-authoring discipline). 25 seed-data tests total, all green. Backend 30 architecture + 2,463 unit + 1,332 integration all green. C1-C2 remain. Not yet imported into any database — Phase 4 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan |
| 2026-07-23 | **Skill Graph rebuild Phase 3 — canonical C1 curriculum authored (100 nodes, 0 isolated, 84 cross-level edges — highest ratio yet).** Fifth of 6 CEFR-level authoring passes. `src/LinguaCoach.Persistence/Seed/SkillGraph/c1.json` — 100 nodes across all 9 skills (Grammar 21, Vocabulary 20, Reading/Listening/Writing 10 each, Speaking 12, Pronunciation 7, Fluency 5, Confidence 5). 103 prerequisite edges: 27 cross-skill, 84 cross-level back to real B1/B2 keys — the highest cross-level ratio of any level so far, since C1 grammar in particular is almost entirely a direct sophistication of B2 structures (inversion after negative adverbials from B2's basic inversion; nominal relative clauses from non-defining relative clauses; native-like ellipsis from B2's ellipsis/substitution). Zero isolated nodes on the first pass, no fix step needed. 31 seed-data tests total, all green. Backend 30 architecture + 2,469 unit + 1,332 integration all green. C2 is the last level remaining. Not yet imported into any database — Phase 4 | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan |
| 2026-07-23 | **Skill Graph rebuild Phase 3 COMPLETE — canonical C2 curriculum authored, closing out the full 6-level, 600-node curriculum.** Sixth and final CEFR-level authoring pass. `src/LinguaCoach.Persistence/Seed/SkillGraph/c2.json` — 100 nodes across all 9 skills (Grammar 21, Vocabulary 20, Reading/Listening/Writing 10 each, Speaking 12, Pronunciation 7, Fluency 5, Confidence 5). 100 prerequisite edges: 5 cross-skill, 94 cross-level back to real C1 keys — by far the highest cross-level ratio of any level, since C2 is deliberately framed as mastery/refinement of C1 structures (native-like prosody, full register command, stylistic mastery) rather than new grammar rules, matching the actual CEFR C2 descriptor ("express spontaneously, very fluently and precisely, differentiating finer shades of meaning"). Zero isolated nodes, no fix pass needed. **Phase 3 grand total: 600 hand-authored nodes across all 6 CEFR levels** (A1 99, A2 100, B1 101, B2 100, C1 100, C2 100), built from the standard CEFR progression sequenced the way mainstream coursebooks order topics — every level validated for taxonomy compliance, no duplicate/dangling keys, no cycles (whole-graph DFS across all 6 files at once), zero fully-isolated nodes. Cross-level linking density increases by level (A2→A1 47 edges, B1→earlier 52, B2→earlier 72, C1→earlier 84, C2→earlier 94), matching how CEFR progression genuinely compounds. `SkillGraphSeedDataTests` now covers 37 checks across all 6 files. Backend 30 architecture + 2,475 unit + 1,332 integration all green. This is the actual dataset Phase 4 (reseed execution) will import via Phase 1's `nodes/import` endpoint — not yet imported into any database | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan. Next: Phase 4 (full wipe-and-reseed of the live 219-node graph with this 600-node canonical dataset) |
| 2026-07-23 | **Skill Graph rebuild Phase 4 COMPLETE — full reseed executed live, isolated-island defect confirmed fixed in real production data.** Executed the reseed against the real Docker dev DB (`linguacoach_dev`), not just validated in files. Rebuilt/redeployed the API container (`--no-cache`) to pick up Phases 1-3. Took a full `pg_dump` backup of the 3 affected tables before any deletion (Sprint 14 precedent for destructive DB changes). Deleted the old 219-node/16-edge graph plus its 115 Module links in FK-safe order, imported all 6 canonical seed files (600 nodes total) via the real `POST nodes/import` endpoint — 0 errors, 0 dropped edges — then batch-approved all 600 nodes and re-ran `retag-modules` (~20 sweep iterations) to relink real content. **Found and fixed a second real bug live**: `RetagModules`'s candidate query compared `SkillGraphNode.Skill` (always lower-invariant) against `Module.Skill` (preserves caller casing — real Modules have `"Vocabulary"`, capitalized) case-sensitively, silently matching 0 candidates for every such Module (confirmed live: real modules like "zoo"/"yourself" matched 0 before the fix, real numbers immediately after). Rebuilt/redeployed again; the fix took total matches from 0 to ~197 across the full sweep. **Final live verification**: 600/600 nodes Approved, 655 edges, 0 coverage gaps across all 54 CEFR×Skill combinations, **0 isolated nodes** (down from 195/223), 121/600 nodes now have real linked content (up from 31/219) — and critically, **the actual `GET /graph` payload (the exact data the Cytoscape viz renders) now contains 258 cross-skill edges and 349 cross-level edges out of 655 total**, direct live proof that the original audit's screenshot defect (isolated category islands) is fixed in real data, not just validated in seed files. Full backend suite 30 architecture + 2,475 unit + 1,332 integration all green throughout. Not yet visually spot-checked in the admin UI itself (recommended quick follow-up) | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md` and its approved implementation plan. **This closes the core rebuild (Phases 1-4)** — the isolated-graph defect the audit found is fixed end-to-end, from backend capability through canonical content through live data. Phase 5 (Resource Bank/Lesson/Exercise/Module cleanup against the new graph) and Phase 6 (ongoing admin UX polish — search, suggest-placement) remain, per the approved plan |
| 2026-07-23 | **Skill Graph admin editability + connectivity audit (docs-only, no implementation).** User flagged the graph viz (screenshot) rendering every Skill category as a fully isolated island and asked for an enterprise-level gap audit of admin editability (create/edit/relink/retag/find/manual+AI create/placement suggestion) before Sprint 1-14's Skill Graph can be called production-ready. Confirmed the isolation is a real backend/algorithm defect, not a viz bug: `AdminSkillGraphController.Draft()` (the only node-creation path) resolves `PrerequisiteTitles` into edges only against nodes sharing the exact same `(CefrLevel, Skill)` as the draft request, so a cross-Skill or cross-CEFR-level prerequisite edge is structurally impossible to ever create — the "graph" is up to 48 disconnected islands by construction, corroborating Sprint 13/14.1's own "15 edges / 219 nodes, mostly edge-isolated" finding with the actual root cause. Also found the Skill Graph has **no node editing** (every content field is `private set`, no `PUT`, only `UpdateTags`/`Approve`/`Reject` exist), **no manual node creation** (AI-only via `Draft()`), **no manual prerequisite-edge management** (no `POST`/`DELETE` on `SkillGraphPrerequisiteEdge` anywhere), **no node search** beyond CEFR/skill/status filters, **no node-to-node placement suggestion** (`IModuleSkillGraphTaggingService` only does Module→Node, never Node→Node), edges persisting *before* human batch-approval (contradicting the architecture doc's stated safety gate), and `Reject()` leaving dangling edges on the rejected node. Full findings, priority grouping (P0/P1/P2), and 10 implementation tasks in the review doc | Full detail: `docs/reviews/2026-07-23-skill-graph-admin-editability-and-connectivity-audit.md`. Next recommended action: scope a dedicated implementation plan for cross-cluster edge resolution + node/edge CRUD before any further Skill Graph UI polish — everything else in the audit depends on these three |
| 2026-07-21 | **Platform Reliability Sprint 14.4 implemented — Skill Graph bulk-selection toolbar fix + Google-Maps-style graph navigation.** Two more pieces of live feedback on the same page: (1) the Nodes table's bulk-selection toolbar ("N selected" + reject-reason input + approve/reject buttons) had a visible gap between it and the section header above it. Root cause: it was wrapped in `.sp-admin-settings-row` (`admin-tokens.css`), a class built for settings-list toggle rows (`padding: 10px 0` + `border-bottom`), not a bulk-action toolbar — the padding and border rendered as unwanted detached whitespace. Swapped to `.sp-admin-form-actions` (the same tight flex-row class Resource Bank's own bulk toolbar already uses) and flattened the markup to match; also trimmed the enclosing filter-bar wrapper's top padding (`16px 18px 0` → `12px 18px 0`) so the header-to-content gap is consistent with Content coverage's card. (2) 219 nodes rendered via `cytoscape-cose-bilkent` were impractical to navigate with mouse-wheel-only zoom/pan. Added a Google-Maps-style floating control panel (bottom-right of the canvas) to `SpAdminSkillGraphVizComponent` — Zoom In/Zoom Out/Fit-to-view buttons (`zoomBy()`/`fitToView()`, animated `cy.animate({zoom/fit})`) — plus a "find node by title" search box (`FormsModule`-bound `searchTerm`, case-insensitive substring match over all non-parent nodes) with prev/next match navigation (`jumpToMatch()`) that centers and zooms to the matched node and applies a `.sp-sgv-highlight` orange border style, so a specific node can be found and jumped to directly instead of manually panning/zooming through the whole graph. Added `minZoom: 0.1`/`maxZoom: 4` to the Cytoscape config to bound the new zoom controls. **Verified**: `npx tsc --noEmit` clean; isolated `admin-skill-graph.component.spec.ts` 13/13 passing; production build clean; full Karma suite 154 pre-existing failures / 1,558 passing (within this suite's documented 154–174 flakiness band, no new regressions); rebuilt `--no-cache` `web` image, redeployed, confirmed the deployed bundle's chunks contain both `sp-sgv-zoom-controls` and the corrected `sp-admin-form-actions` toolbar markup; confirmed `/admin/skill-graph` returns HTTP 200 from the redeployed container. | Diagnosed the whitespace bug from CSS alone (comparing `.sp-admin-settings-row`'s actual declared padding/border against Resource Bank's already-working bulk-toolbar pattern) rather than waiting on a screenshot, since the user had already asked "are you stuck?" three times across this session's long Docker builds and a root-cause was directly derivable from the two class definitions without needing a visual reference. Built the zoom/search controls as a floating overlay panel (`position:absolute` within a new `.sp-sgv-canvas-wrap`) rather than a toolbar row above the canvas, matching the actual Google Maps reference the user named — controls that float over the map content, not ones that consume vertical space and shrink the graph's already-constrained viewport |
