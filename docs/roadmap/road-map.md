...
**Accurate as of: 2026-07-09 (Plan-Sync-After-D3 — see §19a for the current phase sequence).
...
Phase D3, Plan-Sync-After-D3) has
...
**Latest step (local, not yet deployed):** Plan-Sync-After-D3 —
Decide Next Bank-First Phase After Full Reading Passage Today Wiring (2026-07-09, docs-only). A
...
**Latest phase completed (local, not yet deployed):** Phase D3 —
...
## 19a. Phase Sequence (as of 2026-07-09, Plan-Sync-After-D3)
...
18. **Phase E8 — Internal Resource Bank Depth Expansion for Grammar, Usage, and Reading Support** — **next recommended implementation phase (Plan-Sync-After-D3, 2026-07-09)**. Goal: expand original, English-only internal bank depth through the existing staging → validation → approval → publish pipeline, focusing on resource types that unlock safer Today composer expansion in D4. **May include**: more `CefrVocabularyEntry` depth (A1–B2); more `CefrGrammarProfileEntry` depth (A1–B2); more `CefrReadingReference` short-reference material; more `CefrReadingPassage` full-passage material if useful; better metadata coverage (CEFR, skill, subskill where supported, context tags, focus tags, difficulty, estimated time, source/provenance); validation coverage proving published resources are discoverable by the selector. **Must not**: import external datasets; add Persian/bilingual/support-language content; seed final published tables directly; rewrite the Today composer; remove any legacy fallback; delete the readiness/delivery queue; start PG-v2; redesign student UI. Not started.
19. **Phase E8/D4 decision checkpoint** — **resolved (2026-07-09, Plan-Sync-After-D3): Phase E8 first, then Phase D4.** A fresh checkpoint follows E8 to confirm D4 (vs PG-v2A or Phase G2/G3) is still the right next step given what E8 produces. Not reached yet.
20. **Phase D4 — Broader Today Bank-First Composer Expansion** — the likely composer phase *after* E8 (not cancelled). Likely areas: use richer grammar/vocabulary/reading bank bundles; improve pattern-specific bank context; expand safe bank-first coverage beyond the current vocabulary/reading slice — **keeping legacy fallback, not rewriting the full Today composer**, and only where bank resources are deep enough. Gated on E8 delivering richer bank material. Not started.
20a. **Phase PG-v2A** — alternatively selectable at the post-E8 checkpoint; see item 21 below. Not started.
...
and deepening the bank first makes D4 safer/more useful and reduces AI-fallback pressure. Phase D4
remains the likely composer phase after E8, not cancelled. **Phase E8 (recommended next), Phase D4,
Phase G2/G3 (backend/diagnostics cleanup), and PG-v2 implementation have not started.**
