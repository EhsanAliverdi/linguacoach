...
**Accurate as of: 2026-07-09 (Plan-Sync-After-D4 — see §19a for the current phase sequence).
...
Phase D3, Plan-Sync-After-D3, Phase E8, Phase D4, Plan-Sync-After-D4) has
...
**Latest step (local, not yet deployed):** Plan-Sync-After-D4 —
Decide Next Phase After Richer Today Bank-First Composition (2026-07-09, docs-only). A docs-only
...
**Latest phase completed (local, not yet deployed):** Phase D4 —
...
## 19a. Phase Sequence (as of 2026-07-09, Plan-Sync-After-D4)
...
20b. **Phase E9 — Published Bank Metadata Parity for Context-Aware Selection** — **next recommended implementation phase (Plan-Sync-After-D4, 2026-07-09)**. Goal: give `CefrVocabularyEntry`/`CefrGrammarProfileEntry`/`CefrReadingReference` the same published metadata `CefrReadingPassage` already has (context/focus tags, subskill, difficulty band as appropriate), so Today/D5 and a future PG-v2 selector can filter all bank types by context/focus/subskill/difficulty consistently. **Scope**: add metadata columns to those three final tables aligned with `CefrReadingPassage`; extend `ResourceCandidatePublishService` to map candidate metadata into the final tables at publish; backfill existing internal E6/E7/E8 rows from candidate/provenance metadata where safely traceable; extend the E5 browse/search query surface only where filters already exist or need a small safe extension; add tests for mapping, backfill, filtering, and discoverability. **Must not**: add a new seed pack (beyond tiny test fixtures); import external datasets; add Persian/bilingual/support-language content; seed final tables directly; rewrite the Today composer; start PG-v2; rewrite Practice Gym; remove any legacy fallback; delete the readiness/delivery queue; redesign student/admin UI. Not started.
20c. **Phase D5 — Context-Aware Today Bank Selection and Topic Matching** — the likely Today/composer phase *after* E9 (not cancelled). Likely areas: use E9's metadata parity for consistent context/focus/subskill/difficulty filtering across all bank types; improve topic matching within the published banks; reduce irrelevant supporting resources; **keep legacy fallback; avoid semantic/vector search unless explicitly chosen**. Gated on E9 delivering metadata parity. Not started.
20d. **Next-step checkpoint after E9/D5 (not resolved in advance)** — candidates: **Phase D5** (per above), **Phase PG-v2A** (backend skill/objective-first Practice Gym selector; see item 21), **Phase F** (legacy freeform-generation retirement, per-pattern), or **Phase G2/G3** (backend/diagnostics cleanup). Not reached yet.
...
not composer mechanics. Phase D5 (Context-Aware Today Bank Selection and Topic Matching) is the
likely Today phase after E9. **Phase E9 (recommended next), Phase D5, Phase G2/G3
(backend/diagnostics cleanup), and PG-v2 implementation have not started.**
