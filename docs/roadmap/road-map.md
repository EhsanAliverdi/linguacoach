...
**Latest phase completed (local, not yet deployed):** Phase H0 —
...
20i. **Phase H1 — Unified Resource Bank Admin Read Model** `Planned, not started` — one admin-facing Resource Bank API/page aggregating the existing typed published bank tables (`CefrVocabularyEntry`/`CefrGrammarProfileEntry`/`CefrReadingReference`/`CefrReadingPassage`); no physical consolidation; old typed pages may remain under Advanced temporarily. **Recommended next implementation phase.**
20j. **Phase H2 — Import Content UX v1** `Planned, not started` — admin upload/paste/import page; broad type/category selection; AI analyze/mapping preview; creates pending Resource Candidates/Bank rows through the existing E1–E9 pipeline; async for large imports; no student assignment.
20k. **Phase H3 — Learn Item Foundation** `Planned, not started` — new `Learn Item` entity/table/API/admin review; generate from selected Resource Bank rows; approval lifecycle; tags/source traceability.
20l. **Phase H4 — Activity Foundation with Form.io** `Planned, not started` — align/extend `Activity` (building on `ActivityTemplate`) as an editable generated exercise with Form.io schema/scoring/feedback plan, generated from selected Resource Bank rows; approval lifecycle.
20m. **Phase H5 — Module Foundation** `Planned, not started` — `Module` = Learn + Activity/Activities + Feedback Plan; generate module drafts from selected resources/Learn Items/Activities; approval lifecycle; objective/estimated-time metadata.
