...
**Latest phase completed (local, not yet deployed):** Phase E10 —
...
internal packs (a narrowed residual `TODO-E10-1` notes the runtime job still null-feeds
subskill/difficulty preferences — a D6 concern). **Phase D6 (Today Topic Matching and Subskill-Aware
Resource Selection) is the likely next phase**; PG-v2, Phase F, and G2/G3 remain later. Full detail:
...
20f. **Phase D6 — Today Topic Matching and Subskill-Aware Resource Selection** — **the likely next implementation phase (now unblocked by E10's metadata depth)**. Likely areas: use the richer E10 metadata for stronger topic/focus/subskill matching in `TodayBankResourceSelector`; improve supporting-resource relevance; reduce irrelevant vocabulary/grammar pairings; feed `PreferredSubskill`/`PreferredDifficultyBand` at runtime from a reliable per-request source (`TODO-E10-1`); **keep deterministic metadata matching (no embeddings/vector search unless explicitly chosen); preserve legacy fallback**. Not started.
20g. **Next-step checkpoint after E10 (now live, not resolved in advance)** — candidates: **Phase D6** (per above), **Phase PG-v2A** (backend skill/objective-first Practice Gym selector; see item 21, expected to benefit from E9 parity + D5 wiring + E10 depth + D6 selection), **Phase F** (legacy freeform-generation retirement, per-pattern), or **Phase G2/G3** (backend/diagnostics cleanup). Not reached yet.
