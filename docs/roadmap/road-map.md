...
**Accurate as of: 2026-07-09 (Plan-Sync-After-D5 — see §19a for the current phase sequence).
...
Phase D3, Plan-Sync-After-D3, Phase E8, Phase D4, Plan-Sync-After-D4, Phase E9, Phase D5, Plan-Sync-After-D5) has
...
**Latest step (local, not yet deployed):** Plan-Sync-After-D5 —
Decide Next Phase After Context-Aware Today Selection (2026-07-09, docs-only). A docs-only
...
**Latest phase completed (local, not yet deployed):** Phase D5 —
...
## 19a. Phase Sequence (as of 2026-07-09, Plan-Sync-After-D5)
...
20e. **Phase E10 — Internal Bank Metadata Depth Expansion for Focus and Difficulty** — **next recommended implementation phase (Plan-Sync-After-D5, 2026-07-09)**. Goal: enrich existing internal published lean rows (`CefrVocabularyEntry`/`CefrGrammarProfileEntry`/`CefrReadingReference`) with stronger focus tags, difficulty bands, and subskill coverage (using the `CurriculumSubskillConstants` taxonomy) so Today (D6) and future PG-v2 selectors have enough signal to choose relevant resources. **Scope**: audit E6/E7/E8 metadata coverage; enrich/repair metadata for existing internal rows **where safely traceable** through the existing staging → validation → approval → publish path or the idempotent safe metadata-repair path (`PublishedBankMetadataBackfillSeeder`-style); keep everything English-only and source-traceable; add tests for coverage, traceability, idempotency, and selector discoverability. **Must not**: import external datasets; add Persian/bilingual/support-language content; seed final tables directly; add schema/migrations (E9's columns already exist); rewrite the Today composer; start PG-v2; rewrite Practice Gym; remove any legacy fallback; delete the readiness/delivery queue; redesign student/admin UI. Not started.
20f. **Phase D6 — Today Topic Matching and Subskill-Aware Resource Selection** — the likely Today/composer phase *after* E10 (not cancelled). Likely areas: use the richer E10 metadata for stronger topic/focus/subskill matching in `TodayBankResourceSelector`; improve supporting-resource relevance; reduce irrelevant vocabulary/grammar pairings; **keep deterministic metadata matching (no embeddings/vector search unless explicitly chosen); preserve legacy fallback**. Gated on E10 delivering metadata depth. Not started.
20g. **Next-step checkpoint after E10/D6 (not resolved in advance)** — candidates: **Phase D6** (per above), **Phase PG-v2A** (backend skill/objective-first Practice Gym selector; see item 21, expected to benefit from E9 parity + D5 wiring + E10 depth + D6 selection), **Phase F** (legacy freeform-generation retirement, per-pattern), or **Phase G2/G3** (backend/diagnostics cleanup). Not reached yet.
...
matching and a future PG-v2 selector materially better. Phase D6 (Today Topic Matching and
Subskill-Aware Resource Selection) is the likely Today phase after E10. **Phase E10 (recommended
next), Phase D6, Phase G2/G3 (backend/diagnostics cleanup), and PG-v2 implementation have not
started.**
