...
**Accurate as of: 2026-07-09 (Phase E9 — see §19a for the current phase sequence).
...
Phase D3, Plan-Sync-After-D3, Phase E8, Phase D4, Plan-Sync-After-D4, Phase E9) has
...
**Latest phase completed (local, not yet deployed):** Phase E9 —
Published Bank Metadata Parity for Context-Aware Selection (2026-07-09). Closed the D4-discovered
...
**Latest step before this:** Plan-Sync-After-D4 —
...
## 19a. Phase Sequence (as of 2026-07-09, Phase E9)
...
20c. **Phase D5 — Context-Aware Today Bank Selection and Topic Matching** — **the likely next implementation phase (now unblocked by E9's metadata parity)**. Likely areas: use E9's metadata parity for consistent context/focus/subskill/difficulty filtering across all bank types in `TodayBankResourceSelector`; improve topic matching within the published banks; reduce irrelevant supporting resources; **keep legacy fallback; avoid semantic/vector search unless explicitly chosen**. Not started.
20d. **Next-step checkpoint after E9 (now live, not resolved in advance)** — candidates: **Phase D5** (per above), **Phase PG-v2A** (backend skill/objective-first Practice Gym selector; see item 21), **Phase F** (legacy freeform-generation retirement, per-pattern), or **Phase G2/G3** (backend/diagnostics cleanup). Not reached yet.
...
filters), closing `TODO-D4-1` for those tables. **Phase E9 is complete.** Phase D5 (Context-Aware
Today Bank Selection and Topic Matching) is the likely next phase, now unblocked. **Phase D5,
