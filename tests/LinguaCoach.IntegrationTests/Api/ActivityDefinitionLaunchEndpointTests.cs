...
/// <summary>
/// Phase H10 — ActivityDefinition Runtime Launch Path / Attempt Bridge. Uses the plain
/// <see cref="ApiTestFactory"/> (matches H7's <c>PracticeGymModulePipelineEndpointTests</c>
/// convention) plus directly-seeded approved Module Definitions with a launch-eligible Activity
/// Definition (well-formed <c>ScoringRulesDocument</c>-shaped JSON, matching what
/// <c>ActivityGenerationService</c> actually produces).
/// </summary>
public sealed class ActivityDefinitionLaunchEndpointTests : IClassFixture<ApiTestFactory>
{
    private readonly ApiTestFactory _factory;

    public ActivityDefinitionLaunchEndpointTests(ApiTestFactory factory) => _factory = factory;
    ...
    private async Task<Guid> SeedLaunchableModuleAsync()
    {
        ...
        var learnItem = new LearnItem($"Learn {Guid.NewGuid():N}", "Body text.", LearnItemSourceMode.Manual, "B1", "Vocabulary");
        learnItem.Approve(null);
        db.LearnItems.Add(learnItem);

        var activity = new ActivityDefinition($"Activity {Guid.NewGuid():N}", "Type the missing word.", "gap_fill",
            ActivityRendererType.Formio, ActivitySourceMode.Manual, cefrLevel: "B1", skill: "Vocabulary", estimatedMinutes: 5,
        ...
        var module = new ModuleDefinition($"Module {Guid.NewGuid():N}", ModuleSourceMode.Manual, cefrLevel: "B1", skill: "Vocabulary");
        module.Approve(null);
    ...
    private async Task<Guid> SeedUnsupportedModuleAsync()
    {
        ...
        var activity = new ActivityDefinition($"Activity {Guid.NewGuid():N}", "Write a paragraph.", "short_answer",
            ActivityRendererType.Formio, ActivitySourceMode.Manual, cefrLevel: "B1", skill: "Vocabulary", estimatedMinutes: 5,
