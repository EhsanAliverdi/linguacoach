...
/// <summary>Phase H5 — Module foundation admin endpoints.</summary>
public sealed class AdminModuleEndpointTests : IClassFixture<ApiTestFactory>
{
    private readonly ApiTestFactory _factory;

    public AdminModuleEndpointTests(ApiTestFactory factory) => _factory = factory;

    private async Task<(Guid LessonId, Guid ActivityId)> SeedApprovedLessonAndActivityAsync(
        string cefrLevel = "B1", string skill = "Vocabulary")
    {
        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<LinguaCoachDbContext>();

        var lesson = new Lesson($"Learn {Guid.NewGuid():N}", "Body text.", LessonSourceMode.Manual, cefrLevel, skill);
        lesson.Approve(null);
    ...
    {
        var (lessonId, activityId) = await SeedApprovedLessonAndActivityAsync();
        var token = await _factory.CreateAdminAndGetTokenAsync();
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
        ...
        var createResp = await client.PostAsJsonAsync("/api/admin/modules", new
        {
            ...
            lessonLinks = new[] { new { lessonId, role = "Primary" } },
            exerciseLinks = new[] { new { exerciseId = activityId, role = "PrimaryPractice" } },
        });
    ...
    public async Task Admin_can_generate_module_from_selected_items()
    {
        ...
        Assert.Equal("GeneratedFromLessonAndExercises", module.GetProperty("sourceMode").GetString());
    }
    ...
    public async Task Admin_can_approve_and_reject_modules()
    {
        ...
        var (lessonId2, activityId2) = await SeedApprovedLessonAndActivityAsync();
        var genResp2 = await client.PostAsJsonAsync("/api/admin/modules/generate-from-items", new
        {
            lessonLinks = new[] { new { lessonId = lessonId2, role = "Primary" } },
            exerciseLinks = new[] { new { exerciseId = activityId2, role = "PrimaryPractice" } },
        });
