...
public sealed class LessonCommandHandlerTests : IDisposable
{
    private readonly LinguaCoachDbContext _db;
    private readonly AdminCreateLessonHandler _createHandler;
    private readonly AdminUpdateLessonHandler _updateHandler;
    private readonly AdminApproveLessonHandler _approveHandler;
    private readonly AdminRejectLessonHandler _rejectHandler;
    ...
    public LessonCommandHandlerTests()
    {
        ...
        _createHandler = new AdminCreateLessonHandler(_db);
        _updateHandler = new AdminUpdateLessonHandler(_db);
        _approveHandler = new AdminApproveLessonHandler(_db);
        _rejectHandler = new AdminRejectLessonHandler(_db);
    }
    ...
    private static CreateLessonCommand ManualCommand(
    ...
    [Fact]
    public async Task Create_manual_lesson_is_pending_review()
    {
    ...
    public async Task Update_changes_draft_fields_and_stays_pending_review()
    {
        ...
        var updated = await _updateHandler.HandleAsync(new UpdateLessonCommand(
    ...
    public async Task Approve_transitions_review_status_to_approved()
    {
        ...
        var approved = await _approveHandler.HandleAsync(new ApproveLessonCommand(created.Id, reviewerId, "Looks good"));
    ...
    public async Task Reject_transitions_review_status_to_rejected_with_reason()
    {
        ...
        var rejected = await _rejectHandler.HandleAsync(new RejectLessonCommand(created.Id, "Inaccurate explanation", null));
    ...
    public async Task Reject_requires_a_reason()
    {
        ...
        var act = async () => await _rejectHandler.HandleAsync(new RejectLessonCommand(created.Id, "", null));

        await act.Should().ThrowAsync<LessonValidationException>();
    }

    [Fact]
    public async Task Editing_an_approved_lesson_is_blocked()
    {
        var created = await _createHandler.HandleAsync(ManualCommand());
        await _approveHandler.HandleAsync(new ApproveLessonCommand(created.Id, null));

        var act = async () => await _updateHandler.HandleAsync(new UpdateLessonCommand(
