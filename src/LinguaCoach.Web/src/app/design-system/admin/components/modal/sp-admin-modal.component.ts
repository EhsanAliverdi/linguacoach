...
export type SpAdminModalSize = 'sm' | 'md' | 'lg' | 'xl' | 'full';
export type SpAdminModalVariant = 'default' | 'danger' | 'form' | 'confirm';

const MODAL_SIZE_MAP: Record<SpAdminModalSize, string> = {
  sm:   '380px',
  md:   '520px',
  lg:   '720px',
  xl:   '960px',
...
@Component({
  ...
  template: `
    @if (open) {
      <!--
        TailAdmin modal (shared/components/ui/modal):
      ...
      <div class="fixed inset-0 flex items-center justify-center z-[99999] p-4">
        <div
          class="sp-modal-panel relative w-full bg-white dark:bg-gray-900 shadow-2xl"
          [class]="panelClasses"
          role="dialog"
        ...
        >
          ...
          <div class="sp-modal-header px-6 pt-6 pb-0">
            @if (variant === 'danger') {
              <div class="sp-modal-danger-icon mb-3">
            ...
            <div class="sp-modal-title text-base font-semibold dark:text-white/90 pr-8" [class]="titleClass">{{ title }}</div>
            @if (subtitle) {
  ...
  styles: [`
    ...
    .sp-modal-panel-default { border-radius:24px; }
    .sp-modal-panel-danger  { border-radius:24px; }
    .sp-modal-panel-form    { border-radius:16px; }
...
export class SpAdminModalComponent {
  ...
  @Input() size: SpAdminModalSize = 'md';
  @Input() variant: SpAdminModalVariant = 'default';
  /** Override max-width explicitly. If set, takes precedence over size. */
  @Input() maxWidth = '';
  @Output() closed = new EventEmitter<void>();

  get effectiveMaxWidth(): string {
    return this.maxWidth || MODAL_SIZE_MAP[this.size] || '520px';
  }

  get panelClasses(): string {
    return `sp-modal-panel-${this.variant}`;
  }

  get titleClass(): string {
    return `sp-modal-title-${this.variant}`;
  }
