...
@Component({
  selector: 'app-onboarding-v2-difficulty',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="sp-card" data-testid="step-difficulty">
      ...
      <div class="space-y-2 mb-6">
        <button
          *ngFor="let opt of step.options"
          ...
          (click)="select(opt)"
          [attr.data-testid]="'difficulty-option-' + opt.key"
        >
          {{ opt.label }}
        </button>
      </div>
...
export class OnboardingV2DifficultyComponent {
  ...
  submit(): void {
    if (!this.selectedKey) {
      this.error = 'Please select a preference.';
      return;
    }
