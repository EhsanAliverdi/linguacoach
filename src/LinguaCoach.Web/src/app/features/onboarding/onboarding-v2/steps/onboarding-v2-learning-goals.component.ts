...
@Component({
  selector: 'app-onboarding-v2-learning-goals',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="sp-card" data-testid="step-learning-goals">
      ...
      <div class="space-y-2 mb-4">
        <button
          *ngFor="let opt of step.options"
          ...
          [attr.data-testid]="'goal-option-' + opt.key"
        >
          {{ opt.label }}
        </button>
      </div>
      <div class="mb-4">
        <input
          ...
          [(ngModel)]="customValue"
          maxlength="200"
          data-testid="learning-goals-custom"
        />
      </div>
...
export class OnboardingV2LearningGoalsComponent {
  ...
  submit(): void {
    if (this.selectedKeys.size === 0 && !this.customValue.trim()) {
      this.error = 'Please select at least one goal or enter a custom goal.';
      return;
    }
    this.error = null;
