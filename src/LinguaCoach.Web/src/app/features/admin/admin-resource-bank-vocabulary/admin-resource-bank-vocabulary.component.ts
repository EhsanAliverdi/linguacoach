...
import {
  ResourceBankVocabularyListItemDto,
...
/**
 * Phase E5 — read-only browse/search for the published CefrVocabularyEntry bank. Browse/search
 * only: no edit or delete actions here — all mutation (approve/reject/publish) happens through
 * the Resource Candidates page (Phase E4).
 */
@Component({
  selector: 'app-admin-resource-bank-vocabulary',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './admin-resource-bank-vocabulary.component.html',
})
export class AdminResourceBankVocabularyComponent implements OnInit {
  items = signal<ResourceBankVocabularyListItemDto[]>([]);
  loading = signal(true);
  error = signal('');
  ...
  detail = signal<ResourceBankVocabularyDetailDto | null>(null);
  ...
  loadAll(): void {
    ...
    this.bankSvc.listVocabulary(this.page(), this.pageSize, this.searchQuery() || undefined, cefrLevel, sourceId).subscribe({
      next: result => {
        this.items.set(result.items);
        this.totalCount.set(result.totalCount);
        this.loading.set(false);
      },
      error: err => { this.loading.set(false); this.error.set(err.error?.error ?? 'Could not load vocabulary bank entries.'); },
    });
  }
  ...
  openDrawer(item: ResourceBankVocabularyListItemDto): void {
    this.detail.set(null);
    this.detailError.set('');
    this.drawerOpen.set(true);
    this.detailLoading.set(true);
    this.bankSvc.getVocabularyDetail(item.id).subscribe({
  ...
  rowActions(_item: ResourceBankVocabularyListItemDto): SpAdminRowAction[] {
    return [{ id: 'view', label: 'View', icon: 'view', tone: 'default' }];
  }

  onRowAction(actionId: string, item: ResourceBankVocabularyListItemDto): void {
