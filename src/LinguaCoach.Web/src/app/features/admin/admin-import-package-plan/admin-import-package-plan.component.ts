...
export class AdminImportPackagePlanComponent implements OnInit {
  ...
  // ── Phase 4.4C — read-only STT operation ledger visibility ───────────────────────────────
  sttOperations = signal<ImportSttOperationSummaryDto[] | null>(null);
  sttOperationsLoading = signal(false);
  sttOperationsError = signal('');

  // ── Phase 4.4D — read-only AI candidate-enrichment operation ledger visibility ────────────
  aiOperations = signal<ImportAiEnrichmentOperationSummaryDto[] | null>(null);
  aiOperationsLoading = signal(false);
  aiOperationsError = signal('');
    ...
    this.packageSvc.getManifest(this.packageId).subscribe({
      next: manifest => {
        ...
        this.packageSvc.getPlan(this.packageId).subscribe({
          next: plan => {
            this.plan.set(plan);
            this.rebuildForm(plan);
            this.loading.set(false);
            this.loadSttOperations(plan.planId);
            this.loadAiOperations(plan.planId);
          },
  ...
  /** Phase 4.4C — loads the read-only STT operation ledger summary for the current plan. Never
   *  blocks the rest of the page; failures show inline in their own section. */
  loadSttOperations(planId: string): void {
    this.sttOperationsLoading.set(true);
    this.sttOperationsError.set('');
    this.packageSvc.getSttOperations(this.packageId, planId).subscribe({
      next: rows => { this.sttOperations.set(rows); this.sttOperationsLoading.set(false); },
      error: err => {
        this.sttOperations.set(null);
        this.sttOperationsError.set(err.error?.error ?? 'Could not load STT operations for this plan.');
        this.sttOperationsLoading.set(false);
      },
    });
  }

  /** Phase 4.4D — loads the read-only AI enrichment operation ledger summary for the current
   *  plan. Never blocks the rest of the page; failures show inline in their own section. */
  loadAiOperations(planId: string): void {
    this.aiOperationsLoading.set(true);
    this.aiOperationsError.set('');
    this.packageSvc.getAiOperations(this.packageId, planId).subscribe({
      next: rows => { this.aiOperations.set(rows); this.aiOperationsLoading.set(false); },
      error: err => {
        this.aiOperations.set(null);
        this.aiOperationsError.set(err.error?.error ?? 'Could not load AI operations for this plan.');
        this.aiOperationsLoading.set(false);
      },
    });
  }
  ...
  generatePlan(): void {
    ...
    this.packageSvc.generatePlan(this.packageId, changeReason).subscribe({
      next: plan => {
        this.plan.set(plan); this.rebuildForm(plan); this.regenerating.set(false);
        this.loadSttOperations(plan.planId); this.loadAiOperations(plan.planId);
      },
      error: err => {
