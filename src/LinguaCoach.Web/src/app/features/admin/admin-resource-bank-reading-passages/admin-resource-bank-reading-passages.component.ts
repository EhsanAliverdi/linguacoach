...
import {
  ResourceBankReadingPassageListItemDto,
...
/**
 * Phase E7 — read-only browse/search for the published CefrReadingPassage bank: full-length
 * original/internal (or explicitly license-approved) English reading passages, distinct from the
 * short excerpt/citation-only CefrReadingReference bank (see admin-resource-bank-reading-references).
 * Browse/search only — all mutation happens through the Resource Candidates page (Phase E4).
 */
@Component({
  selector: 'app-admin-resource-bank-reading-passages',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './admin-resource-bank-reading-passages.component.html',
})
export class AdminResourceBankReadingPassagesComponent implements OnInit {
  items = signal<ResourceBankReadingPassageListItemDto[]>([]);
  loading = signal(true);
  error = signal('');
  ...
  detail = signal<ResourceBankReadingPassageDetailDto | null>(null);
  ...
  loadAll(): void {
    ...
    this.bankSvc.listReadingPassages(this.page(), this.pageSize, this.searchQuery() || undefined, cefrLevel, sourceId).subscribe({
      next: result => {
        this.items.set(result.items);
        this.totalCount.set(result.totalCount);
        this.loading.set(false);
      },
      error: err => { this.loading.set(false); this.error.set(err.error?.error ?? 'Could not load reading passage bank entries.'); },
    });
  }
  ...
  openDrawer(item: ResourceBankReadingPassageListItemDto): void {
    this.detail.set(null);
    this.detailError.set('');
    this.drawerOpen.set(true);
    this.detailLoading.set(true);
    this.bankSvc.getReadingPassageDetail(item.id).subscribe({
  ...
  rowActions(_item: ResourceBankReadingPassageListItemDto): SpAdminRowAction[] {
    return [{ id: 'view', label: 'View', icon: 'view', tone: 'default' }];
  }

  onRowAction(actionId: string, item: ResourceBankReadingPassageListItemDto): void {
