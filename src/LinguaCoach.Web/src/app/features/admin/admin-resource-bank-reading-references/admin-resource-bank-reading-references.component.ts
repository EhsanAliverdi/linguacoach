...
import {
  ResourceBankReadingReferenceListItemDto,
...
/**
 * Phase E5 — read-only browse/search for the published CefrReadingReference bank. These rows are
 * short excerpts/citations only, not full reading passages (see CefrReadingReference's own doc
 * comment / ResourceCandidatePublishService's ReadingPassage handling). Browse/search only — all
 * mutation happens through the Resource Candidates page (Phase E4).
 */
@Component({
  selector: 'app-admin-resource-bank-reading-references',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './admin-resource-bank-reading-references.component.html',
})
export class AdminResourceBankReadingReferencesComponent implements OnInit {
  items = signal<ResourceBankReadingReferenceListItemDto[]>([]);
  loading = signal(true);
  error = signal('');
  ...
  detail = signal<ResourceBankReadingReferenceDetailDto | null>(null);
  ...
  loadAll(): void {
    ...
    this.bankSvc.listReadingReferences(this.page(), this.pageSize, this.searchQuery() || undefined, cefrLevel, sourceId).subscribe({
      next: result => {
        this.items.set(result.items);
        this.totalCount.set(result.totalCount);
        this.loading.set(false);
      },
      error: err => { this.loading.set(false); this.error.set(err.error?.error ?? 'Could not load reading reference bank entries.'); },
    });
  }
  ...
  openDrawer(item: ResourceBankReadingReferenceListItemDto): void {
    this.detail.set(null);
    this.detailError.set('');
    this.drawerOpen.set(true);
    this.detailLoading.set(true);
    this.bankSvc.getReadingReferenceDetail(item.id).subscribe({
  ...
  rowActions(_item: ResourceBankReadingReferenceListItemDto): SpAdminRowAction[] {
    return [{ id: 'view', label: 'View', icon: 'view', tone: 'default' }];
  }

  onRowAction(actionId: string, item: ResourceBankReadingReferenceListItemDto): void {
    if (actionId === 'view') this.openDrawer(item);
  }

  /** Short excerpt preview for the list table — the row detail drawer shows the full excerpt. */
  excerptPreview(excerpt: string | null): string {
    if (!excerpt) return '—';
