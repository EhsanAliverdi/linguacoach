...
export class ActivityLessonComponent implements OnInit, OnDestroy {
  ...
  // â”€â”€ Speaking: microphone + recording â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ General submission â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  onRendererSubmit(payload: ExerciseAnswerPayload): void {
    ...
    if (payload.kind === 'freeText') {
    ...
    } else if (payload.kind === 'readingFillInBlanks') {
      submittedContent = JSON.stringify({ answers: payload.answers });
    } else if (payload.kind === 'reorderParagraphs') {
      submittedContent = JSON.stringify({ orderedIds: payload.orderedIds });
    } else if (payload.kind === 'readingWritingFillInBlanks') {
      submittedContent = JSON.stringify({ answers: payload.answers });
    } else if (payload.kind === 'listeningFillInBlanks') {
      submittedContent = JSON.stringify({ answers: payload.answers });
    } else if (payload.kind === 'highlightCorrectSummary') {
      submittedContent = JSON.stringify({ selectedOptionId: payload.selectedOptionId });
    } else if (payload.kind === 'highlightIncorrectWords') {
      ...
      submittedContent = JSON.stringify({ items: payload.items });
    } else if (payload.kind === 'readAloud') {
      submittedContent = JSON.stringify({ items: payload.items });
    } else if (payload.kind === 'repeatSentence') {
      submittedContent = JSON.stringify({ items: payload.items });
    } else if (payload.kind === 'respondToSituation') {
      submittedContent = JSON.stringify({ items: payload.items });
    } else if (payload.kind === 'describeImage') {
      submittedContent = JSON.stringify({ items: payload.items });
    } else if (payload.kind === 'retellLecture') {
      submittedContent = JSON.stringify({ items: payload.items });
    } else if (payload.kind === 'summarizeGroupDiscussion') {
      submittedContent = JSON.stringify({ items: payload.items });
    } else {
  ...
  nextActivity(): void {
    ...
    if (returnTo) {
      // In lesson context â€” go back to the lesson page, not the next activity.
