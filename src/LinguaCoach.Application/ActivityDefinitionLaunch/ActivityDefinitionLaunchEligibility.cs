...
/// <summary>
/// Phase H10 — pure, read-only eligibility check for whether an <see cref="ActivityDefinition"/>
...
/// without a network round trip) and <c>IActivityDefinitionLaunchService</c> (to re-validate
...
/// </summary>
public static class ActivityDefinitionLaunchEligibility
{
    ...
    /// phase — see <see cref="ActivityDefinitionLaunchDecision"/>'s doc comment for why.</summary>
    ...
    public static ActivityDefinitionLaunchEligibilityResult Evaluate(ActivityDefinition activityDefinition)
    {
        if (activityDefinition.ReviewStatus != AdminReviewStatus.Approved)
            return NotEligible("This activity has not been approved yet.");

        if (activityDefinition.RendererType != ActivityRendererType.Formio)
            return NotEligible("This module contains an activity type that is not launchable yet.");

        if (string.IsNullOrWhiteSpace(activityDefinition.FormSchemaJson))
            return NotEligible("This module contains an activity type that is not launchable yet.");

        if (!IsValidJson(activityDefinition.FormSchemaJson))
            return NotEligible("This module contains an activity type that is not launchable yet.");

        if (!SupportedActivityTypes.Contains(activityDefinition.ActivityType))
        ...
        if (RequiresManualOrAiEvaluation(activityDefinition.ScoringRulesJson))
            return NotEligible("This activity requires manual or AI-assisted review and cannot be auto-scored yet.");

        return new ActivityDefinitionLaunchEligibilityResult(true, null);
    }

    private static ActivityDefinitionLaunchEligibilityResult NotEligible(string reason) => new(false, reason);
    ...
    /// <summary>Parses <see cref="ActivityDefinition.ScoringRulesJson"/> (same
...
public sealed record ActivityDefinitionLaunchEligibilityResult(bool CanLaunch, string? UnsupportedReason);
