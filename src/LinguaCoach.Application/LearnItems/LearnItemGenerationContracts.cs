...
// guessed or invented. No AI call is made — see IGenerateLearnItemFromResourcesHandler's doc
...
public sealed record GenerateLearnItemFromResourcesRequest(
...
public sealed record GenerateLearnItemFromResourcesResult(LearnItemDto LearnItem, string ReviewRoute);

/// <summary>
/// Generates a pending-review <see cref="LearnItemDto"/> draft from one or more selected published
...
/// key is out of scope for a foundation phase. The resulting <see cref="LearnItemDto.GenerationProvider"/>
/// is honestly labeled "Deterministic", never a fake AI attribution.
/// </summary>
public interface IGenerateLearnItemFromResourcesHandler
{
    Task<GenerateLearnItemFromResourcesResult> HandleAsync(GenerateLearnItemFromResourcesRequest request, CancellationToken ct = default);
}
