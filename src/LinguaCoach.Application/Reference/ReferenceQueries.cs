...
public sealed record LearningTrackDto(Guid Id, string Name, string Description);
...
public interface IReferenceQueryService
{
    ...
    Task<IReadOnlyList<LearningTrackDto>> GetTracksByLanguagePairAsync(Guid languagePairId, CancellationToken ct = default);
