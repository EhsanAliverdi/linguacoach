...
internal sealed class ActivityDefinitionConfiguration : IEntityTypeConfiguration<ActivityDefinition>
{
    public void Configure(EntityTypeBuilder<ActivityDefinition> builder)
    {
        builder.ToTable("activity_definitions");
        ...
        // No FK constraint to learn_items — LearnItem has no delete path today, and keeping this
        // a soft reference (like ResourceCandidate.PublishedEntityId) avoids coupling Activity
        // foundation's migration to LearnItem's table lifecycle.
        builder.HasIndex(e => e.LearnItemId).HasDatabaseName("ix_activity_definitions_learn_item");
        builder.HasIndex(e => e.ReviewStatus).HasDatabaseName("ix_activity_definitions_review_status");
        builder.HasIndex(e => e.CefrLevel).HasDatabaseName("ix_activity_definitions_cefr_level");
        builder.HasIndex(e => e.Skill).HasDatabaseName("ix_activity_definitions_skill");
        builder.HasIndex(e => e.Subskill).HasDatabaseName("ix_activity_definitions_subskill");
        builder.HasIndex(e => e.ActivityType).HasDatabaseName("ix_activity_definitions_activity_type");
        builder.HasIndex(e => e.CreatedAt).HasDatabaseName("ix_activity_definitions_created_at");
    }
}
