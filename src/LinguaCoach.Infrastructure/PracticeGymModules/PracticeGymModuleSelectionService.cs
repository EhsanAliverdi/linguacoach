...
public sealed class PracticeGymModuleSelectionService : IPracticeGymModuleSelectionService
{
        ...
        PracticeGymModuleSelectionRequest request, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var (nodeMasteryWeaknessModuleIds, unmetPrerequisiteModuleIds) = await ResolveNodeMasteryWeaknessMatchModuleIdsAsync(request.StudentId, pool, ct);
    ...
    /// real, deterministic facts handed to the composer, never inferred by the AI itself.
    ///
    ...
    /// method already needs, rather than paying for a second 200-event mastery evaluation. Mirrors
    /// <see cref="LinguaCoach.Infrastructure.TodayPlanModules.TodayPlanModuleSelectionService"/>'s
    /// identically-named-in-spirit method — two near-duplicate implementations across Today/Gym is
    /// this codebase's own established precedent for this exact pair of services.</summary>
    private async Task<(HashSet<Guid> WeaknessMatchModuleIds, HashSet<Guid> UnmetPrerequisiteModuleIds)> ResolveNodeMasteryWeaknessMatchModuleIdsAsync(
