...
/// (<c>Resource Bank Item → Learn Item/Activity → Module</c>, see
/// docs/architecture/product-model-realignment-h0.md). Generated from one or more selected
/// published Resource Bank rows (see <see cref="LearnItemResourceLink"/> for traceability) or
...
/// convention.
/// </summary>
public sealed class LearnItem : BaseEntity
{
    ...
    public LearnItemSourceMode SourceMode { get; private set; }
    ...
    public LearnItem(
        string title,
        string body,
        LearnItemSourceMode sourceMode,
    ...
    /// <summary>Edits draft content/metadata. Blocked once approved — an approved Learn Item is a
        ...
        int? estimatedMinutes)
    {
        if (ReviewStatus == AdminReviewStatus.Approved)
            throw new InvalidOperationException(
                $"Cannot edit Learn Item '{Title}': it is already approved. Reject it first to reopen editing.");
    ...
    public void Reject(string reason, Guid? reviewedByUserId)
    {
        if (string.IsNullOrWhiteSpace(reason))
            throw new ArgumentException("Reason is required to reject a Learn Item.", nameof(reason));
