...
public sealed class ImportProfile : BaseEntity
{
    ...
    /// <summary>Phase 4.4 — application-checked optimistic concurrency token, regenerated by every
    /// mutator below. Not an EF-native rowversion/xmin token (the codebase's only precedent,
    /// <c>LearningPath</c>'s xmin config, is Postgres-only and not portable to the SQLite test
    /// provider) — the calling service layer (<c>ImportPlanDraftService</c>,
    /// <c>ImportExecutionPlanApprovalService</c>) is responsible for comparing a caller-supplied
    /// "expected" stamp against this value and rejecting the request with a conflict before ever
    /// invoking a mutator, so two admins editing/approving the same plan concurrently cannot
    /// silently overwrite each other.</summary>
    ...
    private void TouchConcurrencyStamp() => ConcurrencyStamp = Guid.NewGuid();
    ...
    public void ReplaceProfileJson(string profileJson)
    {
        ...
        ProfileJson = profileJson;
        TouchConcurrencyStamp();
    }

    /// <summary>Phase 4.4 (Workstream A) — the admin-editing path: unlike
    /// <see cref="ReplaceProfileJson"/> (Draft-only, no estimate update), this is callable while
    /// the plan is Draft or AwaitingApproval (the state plan generation leaves it in — an admin
    /// reviews and edits it there before ever approving) and always updates the estimate alongside
    /// the mapping/routing edit, so the two can never drift out of sync. Bumps
    /// <see cref="ConcurrencyStamp"/> — the caller must have already verified the expected stamp
    /// matched before calling this.</summary>
    public void ReviseDraft(
        string profileJson, int estimatedCandidateCount,
        decimal estimatedCostExpected, decimal estimatedCostMin, decimal estimatedCostMax,
        string? planEstimateJson)
    {
        if (Status is not (ImportProfileStatus.Draft or ImportProfileStatus.AwaitingApproval))
            throw new InvalidOperationException($"Cannot edit an Import Plan in status '{Status}' — only Draft or AwaitingApproval plans are editable.");
        if (string.IsNullOrWhiteSpace(profileJson))
            throw new ArgumentException("ProfileJson is required.", nameof(profileJson));
